...
export default function ClubDetailPage() {
  ...
  async function handleCreateAndAssign() {
    ...
    setIsCreating(true)
    try {
      await client.clubs.createAndAssignMember({ clubId, name: newMemberName.trim(), lastname: newMemberLastname.trim() })
      await loadData(); resetModal()
    } finally { setIsCreating(false) }
  }
  ...
  function resetModal() {
    setIsModalOpen(false); setSearchQuery(""); setSearchResults([])
    setSelectedMember(null); setNewMemberName(""); setNewMemberLastname("")
  }

  function openEdit() {
    if (!club) return
    setEditName(club.name); setEditDescription(club.description)
    setEditSchedule(club.schedule); setEditLocation(club.location)
    setEditOpen(true)
  }
  ...
  async function handleEdit() {
    ...
    setSaving(true)
    try {
      await client.clubs.update({ id: clubId, data: { name: editName.trim(), description: editDescription.trim(), schedule: editSchedule.trim(), location: editLocation.trim() } })
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-[50vh]"><div className="animate-pulse text-muted-foreground">Cargando club...</div></div>
  }
  ...
  if (!club) {
    return (
      <div className="flex flex-col items-center justify-center h-[50vh]">
        ...
        <Link href="/clubs"><Button variant="link">Volver a clubes</Button></Link>
      </div>
    )
  }
      ...
      <Card>
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              ...
              <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {searchResults.map((member) => (
                      <div
                        ...
                        className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-colors ${selectedMember?.id === member.id ? 'border-[#0D5E32] bg-[#0D5E32]/5' : 'border-border hover:bg-muted/50'}`}
                      >
                        <div>
                          <p className="font-medium">{member.name} {member.lastname}</p>
                          <p className="text-sm text-muted-foreground">Registrado: {new Date(member.created_at).toLocaleDateString('es-ES')}</p>
                        </div>
                        {selectedMember?.id === member.id && <Check className="h-5 w-5 text-[#0D5E32]" />}
                      </div>
                    ))}
                  </div>
                )}
                {showCreateForm && (
                  <div className="space-y-4 pt-4 border-t">
                    <p className="text-sm text-muted-foreground">No se encontró ningún miembro. Crea uno nuevo:</p>
                    <div className="space-y-3">
                      <div className="space-y-2">
                        <Label htmlFor="name">Nombre</Label>
                        <Input id="name" placeholder="Nombre" value={newMemberName} onChange={(e) => setNewMemberName(e.target.value)} />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="lastname">Apellido</Label>
                        <Input id="lastname" placeholder="Apellido" value={newMemberLastname} onChange={(e) => setNewMemberLastname(e.target.value)} />
                      </div>
                    </div>
                  </div>
                )}
              ...
              <DialogFooter>
                ...
                {selectedMember ? (
                  <Button onClick={handleAssignMember} disabled={isCreating} className="bg-[#0D5E32] text-white hover:bg-[#0D5E32]/90">
                    {isCreating ? 'Asignando...' : 'Asignar miembro'}
                  </Button>
                ) : showCreateForm && (
                  <Button onClick={handleCreateAndAssign} disabled={isCreating || !newMemberName.trim() || !newMemberLastname.trim()} className="bg-[#0D5E32] text-white hover:bg-[#0D5E32]/90">
        ...
        <CardContent>
          {members.length > 0 ? (
              ...
              <TableBody>
                {members.map((assignment) => (
                  <TableRow key={assignment.id_member}>
                    ...
                    <TableCell>{new Date(assignment.date_assign).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</TableCell>
                    <TableCell>
                      <Button variant="ghost" size="icon" onClick={() => handleRemoveMember(assignment.id_member)} className="text-destructive hover:text-destructive hover:bg-destructive/10">
            ...
            <p className="text-center text-muted-foreground py-8">No hay miembros en este club. Agrega el primero.</p>
          )}
        </CardContent>
      </Card>
